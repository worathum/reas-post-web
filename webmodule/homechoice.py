...
class homechoice():
    ...
    def register_user(self, postdata):
        ...
        response = self.session.http_post(self.site_name+'/insert/insert_user.php', data=datapost)
    ...
    def test_login(self, postdata):
        ...
        response = self.session.http_post(self.site_name+'/chk_login.php', data=datapost)
    ...
    def create_post(self, postdata):
        ...
        if success=="true":
            r = self.session.http_get(self.site_name+'/create_post.html')
            ...
            for each_img in postdata['post_images'][:21]:
                r = self.session.http_post(postimage_url, data={}, files={"file":open(os.getcwd()+"/"+each_img, 'rb')})
            ...
            if success=="true":
                ...
                response = self.session.http_post(self.site_name+'/insert/insert_post.php', data=datapost)
                ...
                if response.status_code==200:
                    ...
                    if len(parsed_response)>1:
                        if parsed_response[1][0]=='1':
                            success = "true"
                            r = self.session.http_get(self.site_name+'/my_property.html')
                            soup = BeautifulSoup(r.text, features=self.parser)
        ...
        if success=="true":
            ...
            for post in all_posts:
                if post.find('a').get('href')==post_url:    
                    ...
                    break
            if flag==1:
                r = self.session.http_get(self.site_name+'/edit_post/'+postdata['post_id']+'.html')
                soup = BeautifulSoup(r.text, features=self.parser)
                teedin_target = soup.find(attrs={"name":"at_target"}).get('value')
                ...
                if success=="true":
                    ...
                    response = self.session.http_post(self.site_name+'/insert/update_post.php', data=datapost)
    ...
    def delete_post(self, postdata):
        ...
        if success=="true":
            ...
            for post in all_posts:
                if post.find('a').get('href')==post_url:    
                    response = self.session.http_get(self.site_name+'/my_property/'+postdata['post_id']+'.html')
                    if response.status_code==200:
    ...
    def search_post(self, postdata):
        ...
        if success == "true":
            ...
            response = self.session.http_get(self.site_name+'/my_property.html')
            if response.status_code==200:
