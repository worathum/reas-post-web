...
class condoable():
    ...
    def register_user(self, userdata):
        ...
        if len(datapost['password']) <= 6:
            ...
            r = self.httprequestObj.http_get('http://condoable.com/checkUserId.do?function=register&userId='+email)
            data = r.text

            if(data == "true"):
                r = self.httprequestObj.http_post('http://condoable.com/signupMember.do', data = datapost)
                data = r.text
                success = "True"
    ...
    def test_login(self, logindata):
        ...
        r = self.httprequestObj.http_post('http://condoable.com/login.do', data=datapost)
        data = r.text
    ...
    def create_post(self, postdata):
        ...
        resp = self.httprequestObj.http_get('http://condoable.com/action/advertise/searchCondoProject.jsp?term=' + project_name)
        ...
        if success == "True":
            if condoprojectid is None:
                ...
                r = self.httprequestObj.http_post('http://condoable.com/advertise/addOtherCondoProject.do?zoneId=20', data=data_Test)
                condoprojectid = ""
                for i in r.json():
            ...
            print(datapost)
            r = self.httprequestObj.http_post('http://condoable.com/addAdvertiseDetail.do', data=datapost)
            print(r.status_code)
            ...
            if authenticityToken and post_id:
                ...
                for i in postdata['post_images']:
                    ...
                    r = self.httprequestObj.http_post('http://condoable.com/uploadFile.do?type=advertiseImage8/'+post_id +'&advertiseId='+post_id,data="",files=filestoup)
                r = self.httprequestObj.http_post('http://condoable.com/addAdvertiseMoreDetail.do',data=datapost)
    ...
    def boost_post(self, postdata):
        ...
        # print(login)
        if success == "True":
            r = self.httprequestObj.http_get('https://www.condoable.com/member', verify=False)
            data = r.text
            print(data)
            csrf = re.findall(r'csrf_token:"\w+',data)
            datapost["csrf_token"] = csrf[0].replace("csrf_token:\"", "")
            if(re.search(r''+post_id,data)):
                r = self.httprequestObj.http_post('https://www.condoable.com/member/product_postpone', data=datapost)
        ...
        if(success == "True"):
            ...
            if r.status_code == 500:
                ...
                if(re.search(r'ไม่พบประกาศนี้ หรือประกาศถูกลบไปแล้ว!',r.text)):
                    ...
                    r = self.httprequestObj.http_post("http://condoable.com/action/advertise/deleteAdvertise.jsp",data={"advertiseId": post_id})
    ...
    def del_img(self, all_img):
        for i, img in enumerate(all_img):
            ...
            try:
                del_ = self.httprequestObj.http_post('http://condoable.com/' + str(link), '')
                print(i, del_.text)
            except:
                pass
    def edit_post(self, postdata):
        ...
        if authenticityToken:
            ...
            filestoup = {}
            r_ = self.httprequestObj.http_post('http://condoable.com/listImage.do?id='+ str(post_id) + '&type=advertiseImage&atc=' + str(authenticityToken) + '&sid=78','')
    ...
    def search_post(self, postdata):
        ...
        if success:
            r = self.httprequestObj.http_get('http://condoable.com/myAdvertise.jsp')
            # print(r.url)
            # print(r.status_code)
