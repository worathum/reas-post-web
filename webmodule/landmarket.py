...
class landmarket():
    ...
    def register_user(self, postdata):
        ...
        response = self.session.http_post(self.site_name+'/signup_member.php', data=datapost)     
    ...
    def test_login(self, postdata):
        ...
        response = self.session.http_post(self.site_name+'/login.php', data=datapost)    
        if response.status_code==200:
            soup = BeautifulSoup(response.text, features=self.parser)
    ...
    def create_post(self, postdata):
        ...
        if success=="true":            
            ...
            r = self.session.http_get(self.site_name+'/post')
            if r.status_code==200:
                ...
                if flag:
                    response = self.session.http_post(self.site_name+'/add_property.php', data=datapost, files=files)
                    if response.status_code==200:
                        soup = BeautifulSoup(response.text, features=self.parser)
                        if 'alert-success' in soup.find(class_='alert').get('class'):
                            success = "true"
                            ...
                            r = self.session.http_get(self.site_name+'/maneg_property.php')
                            if r.status_code==200:
                                soup = BeautifulSoup(r.text, features=self.parser)
    ...
    def edit_post(self, postdata):
        ...
        if success=="true":
            ...
            r = self.session.http_get(self.site_name+'/edit_property.php?id='+str(postdata['post_id']))
            if r.status_code==200:
                soup = BeautifulSoup(r.text, features=self.parser)
                flag = True
                code_input = soup.find('input', attrs={'name': 'code'})
                ...
                if flag:
                    response = self.session.http_post(self.site_name+'/edit_property.php?id='+str(postdata['post_id']), data=datapost, files=files)
                    if response.status_code==200:
                        soup = BeautifulSoup(response.text, features=self.parser)
                        if 'alert-success' in soup.find(class_='alert').get('class'):
                            success = "true"
    ...
    def search_post(self, postdata):
        ...
        if success == "true":
            ...
            flag = True
            while flag:
                response = self.session.http_get(self.site_name+'/maneg_property.php?&page='+str(page_num))
                if response.status_code==200:
                    soup = BeautifulSoup(response.text, features=self.parser)
                    posts_element = soup.find(class_='well')             
    ...
    def delete_post(self, postdata):
        ...
        if success=="true":
            ...
            response = self.session.http_get(self.site_name+'/maneg_property.php?delete='+str(postdata['post_id']))
            if response.status_code==200:
                soup = BeautifulSoup(response.text, features=self.parser)
