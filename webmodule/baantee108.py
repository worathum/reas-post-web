...
class baantee108():
    ...
    def __init__(self):
        ...
        self.debug = 0
        self.debugresdata = 0
        self.baseurl = 'http://www.baantee108.com'
    ...
    def register_user(self, postdata):
        ...
        response = self.session.http_post('http://www.baantee108.com/member-register.php', data = data1, headers = headers)
        ...
        if data['email'] == "":
            ...
            try:
                res = self.session.http_post('http://www.baantee108.com/p-member-register.php', data = data, headers = headers)
    ...
    def test_login(self, postdata):
        ...
        if data['email'] == "":
            ...
            try:
                response = self.session.http_post('http://www.baantee108.com/login.php', data = data, headers = headers)
                ...
                if 'ขออภัยครับ ท่านกรอก Email และ/หรือ Password ไม่ถูกต้องครับ' in response.text:
                    ...
                    res = self.session.http_get('http://www.baantee108.com/member/index.php')
                    #print(res.text)
    ...
    def create_post(self,postdata):
        ...
        if (login["success"] == "true"):
            ...
            #data['area'] = str(str(postdata['land_size_rai']) + ' ไร่ ' + str(postdata['land_size_ngan']) + ' งาน ' + str(postdata['land_size_wa']) + ' ตรว ' + str(postdata['floor_area']) + ' ตรม.')

            province = ''.join(map(str,str(postdata['addr_province']).split(' ')))

            find_province = self.session.http_get('http://www.baantee108.com/member/post-property.php', headers = headers).text
            ...
            url_district = str('http://www.baantee108.com/data_for_list3.php?province='+data['province'])
            ...
            respo = self.session.http_get('http://www.baantee108.com/member/post-property.php', headers = headers)
            ...
            crt_post = self.session.http_post('http://www.baantee108.com/member/p-post-property.php', data = data, files = file, headers = headers)
            #print(crt_post.text)
            ...
            post_url = str('http://www.baantee108.com/property-'+post_id+'/.html')

            sec_step_url = str('http://www.baantee108.com/member/real-estate-features.php?post_id='+post_id)
    ...
    def boost_post(self,postdata):
        ...
        if(login['success'] == "true"):
            all_posts_url = 'http://www.baantee108.com/member/list-property.php'

            all_posts = self.session.http_get(all_posts_url, headers = headers)
            # print(all_posts)
            soup = BeautifulSoup(all_posts.content, features = "html.parser")
            try:
                pages = soup.findAll('a',attrs={'class':'paginate'})
                temp = []
                for page in pages:
                    temp.append(page.text)
                max_p = int(temp[-1])
            except:
                max_p = 1
            ...
            for i in range(1,max_p+1):
                ...
                if req_post_id in all_post_ids:
                    boost_url = str('http://www.baantee108.com/member/slide-property.php?post_id='+req_post_id)
        ...
        if(login['success'] == "true"):
            ...
            for i in range(1,max_p+1):
                ...
                if req_post_id in all_post_ids:
                    ...
                    delete_post = self.session.http_post('http://www.baantee108.com/member/manage-property-not-sale.php', data = data, headers = headers)
        ...
        if (login['success'] == 'true'):
            ...
            for i in range(1,max_p+1):
                ...
                if req_post_id in all_post_ids:
                    ...
                    pro_url = str('http://www.baantee108.com/member/edit-property.php?post_id='+req_post_id)
                    ...
                    if 'post_images' in postdata and len(postdata['post_images']) > 0:
                        ...
                        if len(postdata['post_images']) <= 8 and len(postdata['post_images']) >=2:
                            for i in postdata['post_images'][1:]:
                                y=str(random.randint(0,100000000000000000))+".jpg"
                                #print(y)
                                file.append((str('file'+str(temp)), (y, open(i, "rb"), "image/jpg")))
                            ...
                            for i in postdata['post_images'][1:8]:
                                y=str(random.randint(0,100000000000000000))+".jpg"
                                #print(y)
                                file.append((str('file'+str(temp)), (y, open(i, "rb"), "image/jpg")))
                                temp = temp + 1
                        ...
                        edit_post_url = str('http://www.baantee108.com/member/p-edit-property.php')
        ...
        if (login['success'] == 'true'):
            ...
            for i in range(1,max_p+1):
                ...
                for abc in xyz.find_all('tr')[1:-1]:
                    title = str(abc.find('a', attrs = {'target':'_blank'})['title'])
                    # print(title)
                    # print(req_post_title)
                    # print()
                    # print()
                    if req_post_title == title:
                        post_id = str(abc.input['value'])
                        post_url = str('http://www.baantee108.com/property-'+str(post_id)+'/.html')
                        post_found = "true"
                        ...
                        pqr = sou.find('p', attrs = {'class': 'news-time'}).text.split(' ')
