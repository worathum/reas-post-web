...
class onlineoops:
    ...
    def register_user(self, postdata):
        ...
        if response.status_code == 200:
            ...
            detail = "An Error has occurred with response_code " + str(
    ...
    def create_post(self, postdata):
        ...
        if success == "true":
            ...
            postdata["lat_long"] = (
                str(postdata["geo_latitude"]) + "," + str(postdata["geo_longitude"])
            )
            if ("floor_total" not in postdata) or (
                "floor_total" in postdata
                and not str(postdata["floor_total"]).isnumeric()
            ):
                postdata["floor_total"] = "1"
            if ("bed_room" not in postdata) or (
                "bed_room" in postdata and not str(postdata["bed_room"]).isnumeric()
            ):
                postdata["bed_room"] = "1"
            ...
            try:
                ...
                postdata["land_size_wa"] = 0
            postdata["land_size"] = (
                400 * postdata["land_size_rai"]
                + 100 * postdata["land_size_ngan"]
                + postdata["land_size_wa"]
            )
            if postdata["land_size"] == 0:
                if (postdata["floor_area"]).isnumeric() and int(
                    postdata["floor_area"]
                ) > 0:
            ...
            datapost = {
                ...
                "PostCategory[subcate_class]": property_types[
                    str(postdata["property_type"])
                ][0],
            ...
            for i, eachdata in enumerate(
                property_types[str(postdata["property_type"])][1:]
            ):
                if eachdata:
            ...
            if response.status_code == 200:
                ...
                if (
                    ...
                    and "alert-success" in confirmation_message.get("class")
                ):
                    success = "true"
            ...
            if site_error:
                ...
                for i in script:
                    if "fileinput_" in i:
                        data.append((i))
                data[0] = (
                    data[0].split("=")[3].split('initialPreview":[')[1].split("]")[0]
                )
                ...
                if (
                    "land_size_ngan" not in postdata
                    or postdata["land_size_ngan"] == None
                ):
                    postdata["land_size_ngan"] = 0
                if "land_size_rai" not in postdata or postdata["land_size_rai"] == None:
                    postdata["land_size_rai"] = 0
                ...
                for i, eachdata in enumerate(
                    ...
                    try:
                        if eachdata:
                            datapost["Attr[" + str(i) + "][tmpvalue]"] = postdata[
                                eachdata
                            ]
                        else:
                            datapost["Attr[" + str(i) + "][tmpvalue]"] = eachdata
                ...
                files = {}
                if len(postdata["post_images"]) > 0:
                    files["PostmarketthTH[image]"] = open(
                        os.getcwd() + "/" + postdata["post_images"][0], "rb"
                    )
    ...
    def delete_post(self, postdata):
        ...
        if success == "true":
            ...
            if response.status_code == 200:
                ...
                if confirmation_message and "alert-error" in confirmation_message.get(
                    "class"
                ):
                    success = "false"
                    detail = "No post found with given id"
                ...
                success = "false"
                detail = (
                    "Unble to delete post. An Error has occurred with response_code "
                    + str(response.status_code)
                )
        else:
            detail = "Unable to login"
                ...
                while class_ != None:
                    ...
                    if res_div:
                        ...
                        for post in all_posts:
                            try:
                                title = (
                                    post.find(class_="add-title").a.getText().split("#")
                                )
        ...
        if success == "true":
            ...
            if site_error:
                ...
                datapost = {
                    ...
                    "PostmarketthTH[quality_type]": "second",  # new or second
                    "PostmarketthTH[price]": soup.find(
                        attrs={"name": "PostmarketthTH[price]"}
                    ).get("value"),
                    "PostmarketthTH[detail]": soup.find(
                        attrs={"name": "PostmarketthTH[detail]"}
                    )
                    .getText()
                    .replace("\n", "<br>"),
                    "PostContact[mobile]": soup.find(
                        attrs={"name": "PostContact[mobile]"}
                    ).get("value"),
                    "ajax": "post-form",
                }
                ...
                if response.status_code == 200:
                    ...
                    success = "false"
                    detail = (
                        "Unble to boost post. An Error has occurred with response_code "
                        + str(response.status_code)
                    )
        else:
            detail = "Unable to login"
